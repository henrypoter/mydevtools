      ...
      <xsl:when test="text()">
        <xsl:choose>
          <!--
               ...
               * The value is not a valid JSON number (i.e. '01', '1.' '.4' are not valid JSON numbers.)
          ...
          <xsl:otherwise>
            ...
            <!--  workaround, i make this treat number as string [By Henry Poter] -->
             <xsl:text/>"<xsl:value-of select="json:encode-string(.)"/>"<xsl:text/>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:when>
